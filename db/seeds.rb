...
50.times {
  new_order = Order.create!(user: User.all.sample)
  3.times {
    OrderItem.create!(
      order: new_order,
  ...
  new_order.subtotal = new_order.items.sum(:price)
  new_order.tax_amount = new_order.subtotal * 0.0875
  new_order.total_amount = new_order.subtotal + new_order.tax_amount
  new_order.save!
}